...
var Title = (function () {
    function Title() {
        this.backgroundTransition = new Timer(6);
        this.currentColor = getRandomColor("#", bg_colors);
        this.nextColor = getRandomColor("#", bg_colors);
        this.room = new DummyRoom(0, 0);
        this.reset();
        this.info = true;
        this.game = false;
    }
    ...
    Title.prototype.draw = function () {
        ...
        if (this.info) {
            ...
            alphaWrap(pulseFraction, function () { drawSprite(titleSprites[1], 8, 11.5); });
            drawSprite(titleSprites[0], 2.5, 8.6, true); // the hero should source-over; ze is not of that dimension
            alphaWrap(Math.max(pulseFraction - 0.1, 0), function () {
                ...
                dots.forEach(function (dot) { return drawDot(dot[0], dot[1]); });
                if (_this.game) {
                    fill("#999999");
                    font(3);
